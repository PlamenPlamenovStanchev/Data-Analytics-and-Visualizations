DROP VIEW IF EXISTS vm_dim_employees;
...
SELECT id AS employee_id, emp_name AS "Full Name", department_code 
FROM employees_raw WHERE is_active = true;



CREATE OR REPLACE VIEW vw_dim_Customers AS
SELECT cust_id, company_name, city, country 
FROM customers_raw;


CREATE OR REPLACE VIEW vw_fct_Sales AS
SELECT order_id, employee_id, customer_id, amount, order_date 
FROM sales_raw WHERE status = 'Completed';


CREATE OR REPLACE VIEW vw_fct_Returns AS
SELECT return_id, employee_id, customer_id, return_date, reason 
FROM returns_raw;